...
private:
	...
	double kisum,kimax,kierr,umbral,out;	
...
void Ballbot::update()
{
	...
	if(now>t_next)
	{
		...
		/****Relación para el movimiento de las ruedas****///////////////////
  		...
  		coseno= cos((angulo1*pi)/180);
  		seno= sin((angulo1*pi)/180);
  		m1=coseno*sqrt(pow(seni,2)+pow(cosi*seno,2));
  		coseno= cos(((angulo1+120)*pi)/180);
  		seno= sin(((angulo1+120)*pi)/180);
  		m2=coseno*sqrt(pow(seni,2)+pow(cosi*seno,2));
  		coseno= cos(((angulo1+240)*pi)/180);
  		seno= sin(((angulo1+240)*pi)/180);
		...
		{kisum=0;}
			...
			{
				if(abs(angulo1-angulo1anterior)>=90)
    		{
				kisum-=error*elapsed;
    		}
    			else
