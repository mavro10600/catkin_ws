...
public:
 Messenger();
  ...
  uint8_t available();
  uint8_t write(std::string str_out);
...
private:
  ...
  char buffer_out[MESSENGERBUFFERSIZE];
  uint8_t bufferIndex_out;
  uint8_t bufferLength_out; // The length of the buffer (defaults to 64)
  uint8_t bufferLastIndex_out; // The last index of the buffer
  char* current_out;
  char* last_out;
};
