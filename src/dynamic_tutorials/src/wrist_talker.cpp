#include <dynamic_tutorials/wrist_talker.h>
#include "ros/ros.h"
...
{
	wrist_PID::wrist_PID(ros::NodeHandle nh) : nh_(nh), kp_pos(30), ki_pos(0.1), kd_pos(0), km_pos(10),enable_(true)
	{
	dynamic_reconfigure::Server <dynamic_tutorials::wrist_PIDConfig>::CallbackType cb;
	cb = boost::bind(&wrist_PID::configCallback,this,_1,_2);
	...
	timer1_=nh.createTimer(ros::Duration(1/rate_act),&wrist_PID::timer1Callback,this);
	
	}
	
	void wrist_PID::start()
	{
	...
	wrist_sub=nh_.subscribe("lec",10,&wrist_PID::encoderCb,this);
	offsetSub=nh_.subscribe("offset",10,&wrist_PID::offsetCb,this);
	pos_desSub=nh_.subscribe("pos_des",10,&wrist_PID::desCb,this);
	...
	void wrist_PID::timer1Callback(const ros::TimerEvent &event)
	{
		if(!enable_)
		return;
		//rate=rate_;
	...
	void wrist_PID::configCallback(dynamic_tutorials::wrist_PIDConfig &config, uint32_t level)
	{
...
void wrist_PID::encoderCb(const std_msgs::Int16::ConstPtr& enc)
{
	lec=enc->data;
...
void wrist_PID::offsetCb(const std_msgs::Int16::ConstPtr& offst)
{
...
void wrist_PID::desCb(const std_msgs::Float32::ConstPtr& des)
{
	pos_des=des->data;
